...
class SuperWoo_GitHub_Updater {
    ...
    public function hooks() { add_filter('update_plugins_github.com', [$this, 'check_update_uri'], 10, 4); add_filter('plugins_api', [$this, 'plugin_information'], 10, 3); }
    public function check_update_uri($update, $plugin_data, $plugin_file, $locales) {
        ...
        if (!$version || version_compare($version, SUPERWOO_VERSION, '<=')) { return false; }
    ...
    private function latest_release() {
        $cached = get_site_transient('superwoo_github_latest_release'); if (false !== $cached) { return is_array($cached) ? $cached : []; }
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) { set_site_transient('superwoo_github_latest_release', [], HOUR_IN_SECONDS); return []; }
        $release = json_decode(wp_remote_retrieve_body($response), true); $release = is_array($release) ? $release : []; set_site_transient('superwoo_github_latest_release', $release, HOUR_IN_SECONDS); return $release;
    }
}
