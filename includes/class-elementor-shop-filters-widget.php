...
class SuperWoo_Elementor_Shop_Filters_Widget extends \Elementor\Widget_Base {
    ...
    protected function register_controls() {
        $visibility = SuperWoo_Shop_Filters::visibility_defaults();
        ...
        foreach ([
            ...
            $this->add_control($key, ['label' => $label, 'type' => \Elementor\Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => !empty($visibility[$key]) ? 'yes' : '']);
        }
    ...
    protected function render() {
        ...
        echo SuperWoo_Shop_Filters::render([ // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            ...
            'show_search' => 'yes' === ($settings['show_search'] ?? (!empty($visibility['show_search']) ? 'yes' : '')),
            'show_categories' => 'yes' === ($settings['show_categories'] ?? (!empty($visibility['show_categories']) ? 'yes' : '')),
            'show_price' => 'yes' === ($settings['show_price'] ?? (!empty($visibility['show_price']) ? 'yes' : '')),
            'show_attributes' => 'yes' === ($settings['show_attributes'] ?? (!empty($visibility['show_attributes']) ? 'yes' : '')),
            'show_stock' => 'yes' === ($settings['show_stock'] ?? (!empty($visibility['show_stock']) ? 'yes' : '')),
            'show_sale' => 'yes' === ($settings['show_sale'] ?? (!empty($visibility['show_sale']) ? 'yes' : '')),
            'show_rating' => 'yes' === ($settings['show_rating'] ?? (!empty($visibility['show_rating']) ? 'yes' : '')),
            'show_sort' => 'yes' === ($settings['show_sort'] ?? (!empty($visibility['show_sort']) ? 'yes' : '')),
        ]);
    }
}
