...
= 1.0.231 =
Remove a page-specific Elementor hook that could leave blank space after the review shortcode on mobile.

= 1.0.230 =
Replace review-star label hit areas with explicit buttons so every star selects its own exact value.
...
= 1.0.227 =
Fix a fatal error in the 1.0.226 package caused by references to files not included in the release archive.

= 1.0.226 =
Fix review star clicks selecting the rating below the clicked star.
...
* Preserve Razorpay's existing native product-page Buy Now behavior on desktop.
* Fall back to the standard WooCommerce checkout if Razorpay's cart checkout overlay cannot be opened.

= 1.0.221 =
* Reset Elementor's cached display settings after applying the carousel product limit so the requested number of products is queried.
* Keep pagination state synchronized, honor Pause on Hover, and contain peek-mode slides within the carousel viewport.

= 1.0.220 =
* Apply carousel product limits at Elementor's pre-render-content stage, before its WooCommerce query is built.

= 1.0.219 =
* Package the plugin in the required SuperWoo directory so WordPress can safely replace and activate the existing installation.

= 1.0.218 =
* Apply the Products Carousel product limit before Elementor caches widget display settings, restoring the requested item count and carousel navigation.

= 1.0.217 =
* Enforce Products Carousel limits through Elementor's final query hook, restoring extra slides, navigation arrows, dots, drag, and autoplay.

= 1.0.216 =
* Make Products Carousel query the full Products to Display total instead of being limited by Elementor's native grid rows.

= 1.0.215 =
* Hide legacy cached play/pause controls from Products Carousel output.

= 1.0.214 =
* Refine Products Carousel arrows with a minimal professional design and make the selected Arrow Icon control apply in live preview.

= 1.0.213 =
* Remove the visible play/pause control from Products Carousel autoplay.

= 1.0.212 =
* Preselect the active product category in Shop Filters on category archive pages.

= 1.0.211 =
* Remove duplicate review-card title headings; review content is shown once.

= 1.0.210 =
* Hide native number-input spinner arrows inside mini-cart quantity controls.

= 1.0.209 =
* Make the initial product-page Add button set quantity to one instead of incrementing an existing stale cart line.

= 1.0.208 =
* Record the originating call stack when the same cart line is added twice during one request.

= 1.0.207 =
* Enforce requested quantity inside recursive product-page add hooks before nested and outer additions complete.

= 1.0.206 =
* Correct third-party product-page quantity recursion without re-emitting WooCommerce quantity-update hooks.
